...
def compress_layer_structure(layer_structure):
    ...
    for layer in layer_structure:
        ...
        i = 0
        while i < len(layer):
            if layer[i] != 0 and layer.count(layer[i]) > 1:
                i += layer.count(layer[i])
            else:
                new_layer.append(position)
                i += 1
            position += 1
        new_layer_structure.append(new_layer)

    layer_structure.reverse()
    new_layer_structure.reverse()
    return new_layer_structure
...
class AsymmetricTreeLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        #print(self.layer)
        self.num_weights = len(self.layer)
        ...
        self.kernel_placer = np.zeros([self.num_weights, input_shape[1]])
        self.copy_placer = np.zeros([1, input_shape[1]])
        self.masker = np.zeros((input_shape[1], input_shape[1] - self.num_weights // 2))
        kernel_counter = 0
        masker_counter = 0

        # create vector of ones for leaves and nodes and vector of ones for unaltered inputs
        for i in range(input_shape[1]):
            self.masker[i, i - masker_counter] = 1.
            if self.layer.count(i):
                self.kernel_placer[kernel_counter, i] = 1.
                kernel_counter += 1
                if i == 0 or self.masker[i - 1, i - masker_counter] == 0.:
                    masker_counter += 1
            else:
                self.copy_placer[0, i] = 1.
        ...
        self.masker = tf.sparse.from_dense(tf.convert_to_tensor(self.masker, dtype=tf.float32))
    ...
    def call(self, inputs):
        ...
        x = tf.math.multiply(inputs, multiplier) + tf.math.multiply(inputs, self.copy_placer)

        # TODO: fix leaky_relu in case inputs at leaves can be negative
        x = tf.nn.leaky_relu(tf.sparse.sparse_dense_matmul(x, self.masker), alpha=.01)
...
def create_asymmetric_model(unique_pixels, asymmetry_index=.2, use_bias=False, non_neg=False, learning_rate=1e-03,
                            learning_mode='constant', learning_pars=None,
                            epsilon=1e-08, layer_structure=None):
    model = tf.keras.Sequential()

    if layer_structure is None:
        layer_structure = compress_layer_structure(
            create_asymmetric_tree_structure(unique_pixels, asymmetry_index=asymmetry_index))
        layer_structure.reverse()
    for layer in layer_structure:
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(
        learning_rate=choose_learning_rate(learning_rate, learning_mode, learning_pars), epsilon=epsilon),
        loss=tf.keras.losses.BinaryCrossentropy(reduction=tf.keras.losses.Reduction.AUTO),
        metrics=['binary_crossentropy', 'acc'])

    return model, layer_structure


def choose_learning_rate(rate=1e-3, mode='constant', pars=None):
    if mode == 'constant':
        return rate
    else:
        if pars is None:
            raise Exception("mode is not constant but pars is None.")
        elif mode == 'exponential_decay':
            return tf.keras.optimizers.schedules.ExponentialDecay(pars['initial_learning_rate'], pars["decay_steps"],
                                                                  pars["decay_rate"])
        elif mode == 'triangular2':
            return tfa.optimizers.Triangular2CyclicalLearningRate(pars["initial_learning_rate"],
                                                                  pars["maximal_learning_rate"], pars["step_size"],
                                                                  pars["scale_mode"])


def display_depth(tree_structure, num_pixels):
    image = create_depth_image(tree_structure, num_pixels)
...
def create_depth_image(tree_structure, num_pixels):
