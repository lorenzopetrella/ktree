...
class ImageDataset:
    ...
    def normalize(self, n_bits=8, rescale=False):

        self.images = self.images / (2 ** n_bits - 1)
        if rescale:
            for i in range(len(self.images)):
                self.images[i] = (self.images[i] - min(self.images[i]))/(max(self.images[i])-min(self.images[i]))
    ...
    def repeat(self, structure):
        """
        Repeats pixels of vectorized image according to first layer structure for asymmetric trees
        :param structure: structure of the first layer of the asymmetric tree
        """
        repeats = np.array([], dtype=np.intc)
        for i in range(1, len(self.images[0])+1):
            repeats = np.append(repeats, structure.count(i))

        self.images = self.images.repeat(repeats, axis=1)
