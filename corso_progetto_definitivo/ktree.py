...
bias_filename = 'bias' if bias else 'nobias'
non_negative_filename = 'nonneg' if non_negative else 'real'
...
for j, (t1, t2, ds) in enumerate(classes):
    ...
    for k, trees in enumerate(trees_set):
        ...
        filename_acc = save_folder + f"{trees}tree_{bias_filename}_{non_negative_filename}_{ds}_acc.npy"
        filename_loss = save_folder + f"{trees}tree_{bias_filename}_{non_negative_filename}_{ds}_loss.npy"
        ...
        for i in range(trials):
            ...
            model = create_model(input_size=X_train.shape[1] * trees, num_trees=trees, use_bias=bias,
