...
for j, (t1, t2, ds) in enumerate(classes):
    ...
    for i in range(trials):
        ...
        for k, trees in enumerate(trees_set):
            ...
            #Train and test ktree, assigning loss and acc values (random ordered input)
            loss_curve, acc_curve, loss[j,i,k], acc[j,i,k], model_t, _ = train_test_ktree(model, trainloaders[i],
                                                                                  validloaders[i], testloader, epochs = epochs, randorder=True)
            # Save accuracy and loss arrays
            np.save('./results/ktree_acc_rand_'+str(class_set)+'.npy', acc)
            np.save('./results/ktree_loss_rand_'+str(class_set)+'.npy', loss)
